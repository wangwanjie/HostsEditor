...
struct ReleaseVersionAndEditingTests {
    @Test
    func releaseVersionIgnoresLeadingVAndTrailingZeros() async throws {
        #expect(ReleaseVersion("v1.0") == ReleaseVersion("1.0.0"))
    }

    @Test
    func releaseVersionComparesNumericComponents() async throws {
        #expect(ReleaseVersion("1.2.10") > ReleaseVersion("1.2.9"))
    }

    @Test
    func releaseVersionHandlesPreReleaseSuffixes() async throws {
        #expect(ReleaseVersion("v2.0.0-beta.1") == ReleaseVersion("2.0"))
    }

    @MainActor
    @Test
    func adjustedEditorFontSizeClampsToUpperBound() async throws {
        #expect(
    ...
    @MainActor
    @Test
    func adjustedEditorFontSizeClampsToLowerBound() async throws {
        #expect(
            AppSettings.adjustedEditorFontSize(
    ...
    @MainActor
    @Test
    func sidebarWidthClampsToSupportedRange() async throws {
    ...
    @Test
    func toggleCommentsAddsHashSpaceToSelectedLines() async throws {
    ...
    @Test
    func toggleCommentsRemovesLeadingWhitespaceAndHashWhenUncommenting() async throws {
        let original = "    # 127.0.0.1 example.test\n# 127.0.0.1 api.example.test"
        let result = HostsEditorTextEditing.toggleComments(
            in: original,
            selectedRanges: [NSRange(location: 0, length: (original as NSString).length)]
    ...
    @Test
    func replaceAllMatchesReplacesEveryOccurrence() async throws {
